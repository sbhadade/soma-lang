...
def emit(path):
    ...
    w('/* Phase II/III/IV bridge required — compile with:                    */')
    w('/*   gcc -O3 -march=native -o prog prog.c runtime/soma_bridge.c -lm -lpthread */')
    w('#include <stdio.h>')
    w('#include <stdlib.h>')
    w('#include <string.h>')
    w('#include <math.h>')
    ...
    w('#include <time.h>')
    w('')
    w('/* ── Phase II/III/IV bridge declarations ── */')
    w('void  soma_emot_tag(int agent_id, int r, int c, float valence, float intensity);')
    w('void  soma_decay_protect(int agent_id, int r, int c, int cycles);')
    w('float soma_predict_err(int agent_id, float *input);')
    w('float soma_emot_recall(int agent_id, int r, int c);')
    w('float soma_surprise_calc(float *a, float *b);')
    w('void  soma_goal_set(int agent_id, float *goal_vec);')
    w('float soma_goal_check(int agent_id, float *current_vec);')
    w('float soma_soul_query(int agent_id, float *vec);')
    w('void  soma_meta_spawn(int agent_id, int count, int entry_pc);')
    w('float soma_evolve(int agent_id, int winner_slot);')
    w('void  soma_introspect(int agent_id);')
    w('float soma_terrain_read(int r, int c);')
    w('void  soma_terrain_mark(int r, int c, float valence);')
    w('void  soma_soul_inherit(int dst_agent, int src_agent);')
    w('int   soma_goal_stalled(int agent_id);')
    w('void  soma_cdbg_emit(int agent_id);')
    w('float soma_cdbg_recv(int agent_id);')
    w('void  soma_ctx_switch(int agent_id, int ctx_nibble);')
    w('')
    ...
    w('      { float _msgtmp[VEC_DIM];')
    w('        if(imm & 0x80000000u) {')
    w('          /* data-section ref: load VEC from g_data */')
    w('          int _off=(imm & 0x7fffffffu)/4;')
    w('          for(int _k=0;_k<VEC_DIM;_k++) _msgtmp[_k]=(_off+_k<DATA_FLOATS)?g_data[_off+_k]:0.f;')
    w('        } else { memcpy(_msgtmp, a->R[src], VEC_DIM*sizeof(float)); }')
    w('        if(ag==0xFF) { if(a->parent_id>=0 && a->parent_id<MAX_AGENTS) agent_enqueue(&g_agents[a->parent_id],_msgtmp); else if(g_main_agent) agent_enqueue(g_main_agent,_msgtmp); }')
    w('        else if(ag==0xFE) agent_enqueue(a,_msgtmp);')
    w('        else agent_enqueue(&g_agents[ag],_msgtmp); } break;')
    ...
    w('    case 0x61: /* GOAL_CHECK   */')
    w('      { float _dist = soma_goal_check(a->id, a->R[dst]);')
    w('        /* soul.py returns dist in [0,1]. JZ fires when (int)dist==0,')
    w('         * i.e. when dist < 1.0 (converged). Do NOT scale to 65535. */')
    w('        a->R[dst][0] = _dist; } break;')
    w('    case 0x62: /* SOUL_QUERY   */ a->R[dst][0]=soma_soul_query(a->id, a->R[dst]); break;')
    w('    case 0x63: /* META_SPAWN   */')
    w('      /* BUG FIX: was soma_meta_spawn() stub — spawned no threads,')
    w('       * so BARRIER returned immediately → infinite curiosity loop.')
    w('       * Now emits real FORK into g_agents slots [2..2+count-1]. */')
    w('      { int _mc = (ag > 0) ? ag : 4;  /* count from ag field */')
    w('        soma_meta_spawn(a->id, _mc, (imm-32)/8);')
    w('        for(int _ci=2; _ci<2+_mc && _ci<MAX_AGENTS; _ci++) {')
    w('          if(!g_agents[_ci].started) {')
    w('            Agent* _ch = &g_agents[_ci];')
    w('            memcpy(_ch->R, a->R, sizeof(a->R));')
    w('            /* Mutate goal: add small noise to R0 per candidate */')
    w('            for(int _k=0;_k<VEC_DIM;_k++)')
    w('              _ch->R[0][_k] += ((float)(_ci-2) - _mc/2.0f) * 0.05f;')
    w('            _ch->id=_ci; _ch->parent_id=a->id;')
    w('            _ch->pc=(imm-32)/8; _ch->running=1; _ch->started=1;')
    w('            pthread_mutex_init(&_ch->inbox_lock,NULL);')
    w('            pthread_cond_init(&_ch->inbox_cond,NULL);')
    w('            pthread_create(&_ch->thread,NULL,agent_run,_ch);')
    w('          }')
    w('        }')
    w('      } break;')
    ...
    w('    case 0x69: /* GOAL_STALL   */ if(soma_goal_stalled(a->id)){ a->pc=(imm-32)/8; jumped=1; } break;')
